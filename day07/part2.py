...
def get_hand_with_joker(cards):
    if ("J" in cards):
        return max([get_hand_type(cards.replace("J", c)) for c in CARD_STRENGTH])
    else:
...
sorted_games = sorted(lines,
                      key=lambda x: (get_hand_with_joker(x.split()[0]), [CARD_STRENGTH.index(c) for c in x.split()[0]]))
